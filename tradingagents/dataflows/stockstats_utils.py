...
# Indicator descriptions appended to every rendered window. Moved here (from
# y_finance) so both the yfinance and Schwab technical-indicator paths share one
# source of truth via ``render_indicator_window``.
BEST_IND_PARAMS = {
    # Moving Averages
    "close_50_sma": (
...
def render_indicator_window(
    ...
    indicator: Annotated[str, "technical indicator to render"],
    curr_date: Annotated[str, "the current trading date, YYYY-mm-dd"],
    look_back_days: Annotated[int, "how many days to look back"],
) -> str:
    """Render a per-day indicator window from an already-prepared OHLCV frame.

    Vendor-agnostic: both the yfinance and Schwab technical-indicator paths call
    this after fetching. It does **not** fetch, clean, or look-ahead filter — it
    is a pure DataFrame -> string transform (no import of ``y_finance``, so the
    single-direction import graph stays acyclic).

    Preconditions (caller's responsibility):
      - ``ohlcv_df`` has a ``Date`` column of **tz-naive** ``datetime64`` dtype.
      - Rows are already **cleaned** (numeric coercion / gap fill) and
        **look-ahead trimmed** to ``<= curr_date``.
      - Rows should be ascending by ``Date``; as a cheap safeguard against an
        unsorted caller, this function re-sorts before ``wrap()`` (stockstats
        assumes ascending order).

    Mirrors the previous yfinance bulk path byte-for-byte: the same
    ``BEST_IND_PARAMS`` validation, the same ``wrap(df)[indicator]`` bulk
    computation, and the same "N/A: Not a trading day" wording for missing days.
    A first-listing symbol with fewer than the indicator's window of trading days
    legitimately yields ``N/A`` (e.g. 200 SMA) — that is not an error.
    """
    ...
    # Defensive ascending sort: stockstats' wrap() assumes ordered rows, and we
    # don't want to depend implicitly on the caller having sorted.
    df = ohlcv_df.sort_values("Date")
    df = wrap(df)
    df["Date"] = df["Date"].dt.strftime("%Y-%m-%d")
