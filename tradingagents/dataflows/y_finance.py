...
def get_stock_stats_indicators_window(
    ...
    # Normal path: fetch the cached, cleaned, look-ahead-trimmed OHLCV frame once
    # and render every date from it (the shared, vendor-agnostic renderer). The
    # bulk computation that used to live in ``_get_stock_stats_bulk`` now lives in
    # ``render_indicator_window`` (minus the fetch), so this path is unchanged.
    try:
        return render_indicator_window(
            load_ohlcv(symbol, curr_date), indicator, curr_date, look_back_days
        )
    except NoMarketDataError:
        raise  # Unknown/delisted symbol — let the router emit the sentinel
    except Exception as e:
        print(f"Error getting bulk stockstats data: {e}")
        # Fallback to the original per-day implementation if the bulk path fails.
        # Each day re-fetches via load_ohlcv inside get_stockstats_indicator.
