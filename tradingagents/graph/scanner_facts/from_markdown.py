...
def _process_ticker_row(
    ...
    hedging = any(
        w in implication.lower() for w in ("may", "could", "potential", "if", "uncertain")
    )
...
def _process_sector_macro_row(
    ...
    _emit_macro_node(
        label, node_type, label, source, evidence_text, implication, nodes, edges, node_ids
    )
...
def _emit_macro_relation(
    ...
    hedging = any(
        w in (evidence_text + " " + implication).lower()
...
def _merge_into(
    ...
    seen_edges = {(e["source"], e["relation"], e["target"], e["provenance"]) for e in target_edges}
