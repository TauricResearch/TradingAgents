...
def filter_scanner_context_for_ticker(scanner_context: str, ticker: str) -> str:
    ...
    try:
        ...
        header_match = re.search(
            r"^# SCANNER CONTEXT PACKET:.*?\n.*?Date:.*?\n", scanner_context, re.MULTILINE
        )
        header = (
            header_match.group(0) if header_match else f"# SCANNER CONTEXT PACKET: {ticker}\n\n"
        )
...
def _filter_structured_data(section_text: str, ticker: str) -> str:
    ...
    earnings_filtered = filter_earnings_calendar(
        earnings_raw, ticker, top_n_sector=10, top_n_overall=10
    )
    ...
    if earnings_filtered:
        filtered_parts.append(
            f"### Earnings Calendar (Filtered: Ticker + Top 10 Same Sector)\n{earnings_filtered}"
        )
    if econ_filtered:
        filtered_parts.append(
            f"### Economic Calendar (Top 10 High-Importance Events)\n{econ_filtered}"
        )
...
def filter_earnings_calendar(
    earnings_text: str, ticker: str, top_n_sector: int = 10, top_n_overall: int = 10
) -> str:
    """
...
def filter_economic_calendar(econ_text: str, max_events: int = 10) -> str:
    ...
    for line in lines:
        ...
        # Keep key economic indicators even without explicit importance tags
            ...
            for indicator in ["fomc", "cpi", "gdp", "unemployment", "nfp", "fed"]
        ):
            high_importance.append(line)
        else:
...
def filter_smart_money_for_ticker(smart_money_report: str, ticker: str) -> str:
    ...
    metadata_block, smart_money_body = _extract_smart_money_metadata_block(smart_money_report)
