...
def _esc(text: str, limit: int = 20000) -> str:
    """Escape and truncate text for Confluence storage format.

    Default limit raised to 20k chars per section — TauricTradingAgents
    routinely emits multi-thousand-char analyst reports and we want the
    full thinking on the page, not a summary.
    """
    if not text:
...
def _section(title: str, content: str, limit: int = 20000) -> str:
    """Render one <h2>title</h2><p>content</p> section, eliding empty ones."""
    if not content or not content.strip():
        return ""
    return f"<h2>{title}</h2><p>{_esc(content, limit)}</p>"
...
def _build_html(symbol: str, signal: str, final_state: dict[str, Any],
    ...
    parts.append(
        ...
        f"<strong>Confidence:</strong> {_confidence_bar(confidence)}</p>"
        "<hr/>"
    )

    # Final decision goes first — most important
    parts.append(_section("Final Trade Decision",
                          final_state.get("final_trade_decision", ""), 30000))

    # Analyst reports — the four pillars
    parts.append("<hr/><h1>Analyst Reports</h1>")
    parts.append(_section("Market Report",       final_state.get("market_report", "")))
    parts.append(_section("Sentiment Report",    final_state.get("sentiment_report", "")))
    parts.append(_section("News Report",         final_state.get("news_report", "")))
    parts.append(_section("Fundamentals Report", final_state.get("fundamentals_report", "")))

    # Researcher debate
    parts.append("<hr/><h1>Bull vs Bear Debate</h1>")
    parts.append(_section("Bull Case",                 debate.get("bull_history", "")))
    parts.append(_section("Bear Case",                 debate.get("bear_history", "")))
    parts.append(_section("Full Debate History",       debate.get("history", "")))
    parts.append(_section("Research Manager Decision", debate.get("judge_decision", ""), 10000))

    # Investment / trader plans
    parts.append("<hr/><h1>Investment Plan</h1>")
    parts.append(_section("Research Manager Plan", final_state.get("investment_plan", "")))
    parts.append(_section("Trader Investment Plan", final_state.get("trader_investment_plan", "")))

    # Risk debate
    parts.append("<hr/><h1>Risk Management Debate</h1>")
    parts.append(_section("Aggressive Analyst",   risk_debate.get("aggressive_history", "")))
    parts.append(_section("Conservative Analyst", risk_debate.get("conservative_history", "")))
    parts.append(_section("Neutral Analyst",      risk_debate.get("neutral_history", "")))
    parts.append(_section("Full Risk Debate",     risk_debate.get("history", "")))
    parts.append(_section("Risk Judge Decision",  risk_debate.get("judge_decision", ""), 10000))

    parts.append(
        f"<hr/><p><small>Generated by TauricTradingAgents"
        f" &middot; <time datetime=\"{ts_iso}\">{ts_display}</time></small></p>"
    )
    # Drop empty sections so the page isn't full of "No content recorded."
