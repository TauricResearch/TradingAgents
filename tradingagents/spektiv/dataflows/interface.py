...
from . import fred
from .fred_common import FredRateLimitError
...
VENDOR_METHODS = {
    ...
    # macroeconomic_data
    "get_interest_rates": {
        "fred": _VendorFunctionProxy(fred, 'get_interest_rates'),
    },
    "get_treasury_rates": {
        "fred": _VendorFunctionProxy(fred, 'get_treasury_rates'),
    },
    "get_money_supply": {
        "fred": _VendorFunctionProxy(fred, 'get_money_supply'),
    },
    "get_gdp": {
        "fred": _VendorFunctionProxy(fred, 'get_gdp'),
    },
    "get_inflation": {
        "fred": _VendorFunctionProxy(fred, 'get_inflation'),
    },
    "get_unemployment": {
        "fred": _VendorFunctionProxy(fred, 'get_unemployment'),
    },
    "get_fred_series": {
        "fred": _VendorFunctionProxy(fred, 'get_fred_series'),
    },
}
    ...
    for vendor in fallback_vendors:
        ...
        for impl_func, vendor_name in vendor_methods:
            try:
                ...
                if vendor == "fred":
                    print(f"RATE_LIMIT: FRED rate limit exceeded, falling back to next available vendor")
                    print(f"DEBUG: Rate limit details: {e}")
                # Continue to next vendor for fallback
                continue
