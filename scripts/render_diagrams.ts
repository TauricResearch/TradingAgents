...
async function run(cmd: string[], opts?: { capture?: boolean; cwd?: string }): Promise<boolean> {
...
async function main() {
  ...
  const files = readdirSync(DIAGRAMS_DIR).filter((f) => f.endsWith(".dot") || f.endsWith(".mmd"))
  ...
  for (const file of files.sort()) {
    ...
    const ok = ext === ".dot" ? await renderDot(src, out) : await renderMmd(src, out)
