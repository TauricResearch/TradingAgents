...
After a new run lands, run `scripts/report_workflow.py` to regenerate indices
and validate the homepage. `_site/` is compiled by the repo-local pre-commit
hook, not by the workflow.

```shell
# refresh top-level + per-ticker index pages from docs/<TICKER>/<run>/ folders
uv run python scripts/build_reports_site.py

# one-shot static build into _site/ (same command class as pre-commit)
