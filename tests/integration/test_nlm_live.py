    ...
    try:
        ...
        for s in sources:
            if s.get("title") == test_title:
                ...
                subprocess.run(
                    [NLM_PATH, "source", "delete", NOTEBOOK_ID, s["id"], "-y"], check=False
                )
        ...
        assert found_id_by_name == source_id, (
            f"Failed to find source ID {source_id} by title '{test_title}'\nFound sources: {[s.get('title') for s in sources]}"
        )
        ...
        if "source_id" in locals() and source_id:
            ...
            subprocess.run(
                [NLM_PATH, "source", "delete", NOTEBOOK_ID, source_id, "-y"], check=False
            )
