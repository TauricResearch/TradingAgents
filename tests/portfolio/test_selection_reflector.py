...
def mock_yf_download(monkeypatch):
    def _mock_download(tickers, start, end, **kwargs):
        ...
        df = pd.DataFrame({"AAPL": ticker_closes, "SPY": spy_closes}, index=dates)
...
def test_fetch_price_data_normal(mock_yf_download):
    terminal_return, spy_return, mfe_pct, mae_pct, days_to_peak, top_move_dates = fetch_price_trend(
        "AAPL", "2025-01-01", "2025-01-05"
    )
...
def test_fetch_price_data_single_day(monkeypatch):
    monkeypatch.setattr(
        "yfinance.download", lambda *args, **kwargs: pd.DataFrame({"AAPL": [100.0], "SPY": [400.0]})
    )
    terminal_return, spy_return, mfe_pct, mae_pct, days_to_peak, top_move_dates = fetch_price_trend(
        "AAPL", "2025-01-01", "2025-01-01"
    )
...
def test_fetch_news_summary_weighted(monkeypatch):
    ...
    monkeypatch.setattr(
        "tradingagents.portfolio.selection_reflector.get_company_news", mock_get_company_news
    )
...
def test_generate_lesson_mfe_mae_in_prompt():
    llm = MagicMock()
    llm.invoke.return_value = AIMessage(
        content='{"situation": "a", "screening_advice": "b", "exit_advice": "c", "sentiment": "neutral"}'
    )
...
def test_reflect_on_scan_no_file(monkeypatch):
    llm = MagicMock()
    monkeypatch.setattr(
        "tradingagents.portfolio.selection_reflector.load_scan_candidates", lambda date: []
    )
