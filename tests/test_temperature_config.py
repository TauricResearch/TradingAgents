...
class TestTemperatureEnvOverlay:
    def test_env_sets_temperature(self, monkeypatch):
        import handelsagenten.default_config as dc
        monkeypatch.setenv("HANDELSAGENTEN_TEMPERATURE", "0.2")
        importlib.reload(dc)
        ...
        monkeypatch.delenv("HANDELSAGENTEN_TEMPERATURE", raising=False)
        importlib.reload(dc)
