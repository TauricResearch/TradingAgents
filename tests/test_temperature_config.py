...
class TestTemperatureForwarding:
    @pytest.mark.parametrize(
        ...
        [
            ...
            # This fork retires ``deepseek-chat`` (see reject_retired_deepseek_model
            # in openai_client.py, which suggests deepseek-v4-flash). The upstream
            # case used deepseek-chat, which get_llm() rejects before temperature
            # forwarding is observable; use the fork's supported model instead so
            # the deepseek provider's temperature forwarding is still exercised.
