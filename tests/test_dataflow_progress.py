...
def test_a_share_tushare_call_emits_progress_without_secret_leak(monkeypatch):
    calls = []
    ...
    messages = [event.message for event in events]
    assert any("数据调用开始：get_stock_data | tushare | 002636.SZ | 2026-01-01~2026-01-31" in msg for msg in messages)
