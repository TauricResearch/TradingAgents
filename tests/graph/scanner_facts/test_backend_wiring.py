    ...
    return {
        ...
        "nodes": [
            ...
            },
            {
                ...
                "label": "Technology",
                "aliases": [],
                "provenance": ["sector_summary.md"],
                "evidence": ["Tech sector"],
                "confidence": 0.90,
            },
        ],
        "edges": [
...
def test_macro_scan_summary_json_written_to_market_dir(tmp_path):
    ...
    with (
        patch("agent_os.backend.services.langgraph_engine.get_market_dir", return_value=market_dir),
        patch("agent_os.backend.services.langgraph_engine.append_to_digest"),
        patch(
            "agent_os.backend.services.langgraph_engine.ensure_scanner_graph_facts"
        ) as mock_ensure,
    ):
...
def test_run_pipeline_proceeds_when_graph_facts_missing(tmp_path):
    ...
    with (
        ...
        async def run():
            ...
            events = []
            async for evt in engine.run_pipeline(
                "TESTRUN",
                {
                    "ticker": "ON",
                    "date": SCAN_DATE,
