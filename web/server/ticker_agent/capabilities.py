...
_DEFINED_CAPABILITIES: list[ApiCapability] = [
    ...
    ApiCapability("/api/runs/{run_id}/trace", "GET", "Get real-time trace of events, stages and LLM calls for a run"),
    ApiCapability("/api/runs/{run_id}/health", "GET", "Get run health including subscriber count"),
    ApiCapability("/api/runs/{run_id}", "DELETE", "Delete a run and its data"),
    ...
    ApiCapability("/api/watchlist/{ticker}", "PATCH", "Update a watchlist ticker group assignment"),
    ApiCapability("/api/watchlist/reorder", "PATCH", "Reorder tickers in the watchlist"),
    ...
    ApiCapability("/api/background-runs/{job_id}", "GET", "Get a specific background run job"),
    ...
    ApiCapability("/api/config/defaults", "GET", "Get default configuration values"),
