...
export { type MarketState, DailyState, StateConfig, TransitionMatrix, RegimeSignal, NDaySignal };
...
function runAllTests(): void {
  ...
  const stateTests = [
    ...
    ['Sideways: flat with noise', (() => {
      const returns = Array(20).fill(0).map(() => (Math.random() - 0.5) * 0.02);
      return classifyState(returns) === 'sideways';
    })()],
    ['Bull threshold: exactly +5%', classifyState(Array(19).fill(0).concat([0.05]), { bullThreshold: 0.05 }) === 'bull'],
    ['Bear threshold: exactly -5%', classifyState(Array(19).fill(0).concat([-0.05]), { bearThreshold: -0.05 }) === 'bear'],
  ];
  stateTests.forEach(([name, passed]) => console.log(`  ${passed ? '✓' : '✗'} ${name}`));
  ...
  matrixTests.forEach(([name, passed]) => console.log(`  ${passed ? '✓' : '✗'} ${name}`));

  // Signal tests
  const mTest: TransitionMatrix = {
    bull_to_bull: 0.80, bull_to_sideways: 0.10, bull_to_bear: 0.10,
    sideways_to_bull: 0.20, sideways_to_sideways: 0.60, sideways_to_bear: 0.20,
    bear_to_bull: 0.10, bear_to_sideways: 0.20, bear_to_bear: 0.70,
  };
  const bullSignal = computeSignal(mTest, 'bull');
  const bearSignal = computeSignal(mTest, 'bear');
  const signalTests = [
    ['Bull signal = 0.70', Math.abs(bullSignal.signal - 0.70) < 1e-9],
  ...
  signalTests.forEach(([name, passed]) => console.log(`  ${passed ? '✓' : '✗'} ${name}`));
...
function smokeTest(): void {
  ...
  states.forEach(s => stateCounts[s.state]++);
  ...
  console.log(`2-day forecast: P(bull)=${nDay.forecastPbull.toFixed(4)}, P(bear)=${nDay.forecastPBear.toFixed(4)}`);
