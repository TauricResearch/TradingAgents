...
def get_user_selections():
    ...
    if asset_type.value == "a_share":
        console.print(
            create_question_box(
                "A-share Position",
                "Enter current cost, portfolio weight, shares, horizon, and risk limit",
            )
        )
        cost_text = typer.prompt("Cost basis (blank if no position)", default="").strip()
        weight = typer.prompt("Current portfolio weight (%)", default=0.0, type=float)
        shares = typer.prompt("Current shares", default=0, type=int)
        horizon = typer.prompt("Holding horizon (trading days)", default=120, type=int)
        max_drawdown = typer.prompt("Maximum acceptable drawdown (%)", default=12.0, type=float)
        bought_today = typer.confirm("Were any of these shares bought today?", default=False)
        portfolio_context = {
            "cost_basis": float(cost_text) if cost_text else None,
            "position_pct": weight,
            "shares": shares,
...
def run_analysis(checkpoint: bool | None = None):
    ...
    with Live(layout, refresh_per_second=4):
        ...
        a_share_context = {}
        if selections["asset_type"] == "a_share":
            from tradingagents.a_share import build_a_share_analysis_context

            a_share_context = build_a_share_analysis_context(
                selections["ticker"],
                selections["analysis_date"],
                selections.get("portfolio_context"),
            )
        init_agent_state = graph.propagator.create_initial_state(
            ...
            portfolio_context=selections.get("portfolio_context"),
