...
def normalize_symbol(raw: str) -> str:
    ...
    # Mainland A shares: accept the common bare six-digit input and broker
    # ``.SH`` suffix, then convert to Yahoo's ``.SS`` convention.
    if re.fullmatch(r"\d{6}(?:\.(?:SH|SS|SZ|BJ))?", s):
        from tradingagents.a_share.context import yahoo_a_share_symbol

        return yahoo_a_share_symbol(s)
