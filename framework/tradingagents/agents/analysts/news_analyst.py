...
def create_news_analyst(llm):
    def news_analyst_node(state):
        ...
        if is_a_share:
            tools = [get_news]
            system_message = (
                "You are an A-share company-news and China-market researcher. "
                "Use get_news only for company-specific items and use the injected "
                "China market environment for market context. Prioritize dated facts, "
                "distinguish official disclosures from media interpretation, and state "
                "when no reliable company news is available. Do not use FRED, Federal "
                "Reserve indicators, Polymarket, Reddit, or StockTwits.\n"
                + render_a_share_context(state.get("a_share_context"))
                + "\nAppend a Markdown evidence table."
                + get_language_instruction()
            )
        else:
            tools = [
