...
async def debug_run():
    ...
    params = {"mock_type": "pipeline", "ticker": "AAPL", "date": "2026-04-06", "speed": 10.0}
    ...
    params = {"ticker": "AAPL", "date": "2026-04-06", "run_id": run_id}
    ...
    print(
        f"Research Manager prompt contains 'Market is bullish at $190': {'Market is bullish at $190' in prompt_text}"
    )
    ...
    print(
        f"Market Structured - Numeric Mentions: {market_struct.get('key_metrics', {}).get('numeric_mentions')}"
    )
    ...
    print(
        f"RM Structured - Confidence (High): {rm_struct.get('key_metrics', {}).get('high_confidence_claims')}"
    )
