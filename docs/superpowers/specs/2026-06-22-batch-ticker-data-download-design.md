...
```
User clicks "Download data" in RunHistoryMenu
    |
    ▼
Frontend: GET /api/tickers/AAPL/download
    |
    ▼
Backend: zip_ticker_data("AAPL")
    ├── Walk data/AAPL/ directory
    ├── Generate summary.csv from run.json files
    ├── Package into ZIP in memory (BytesIO / tempfile)
    └── Return as StreamingResponse
    |
    ▼
Frontend: Blob download via URL.createObjectURL

─────────────────────────────────────────────

User opens Batch Download dialog → checks "Select all"
    |
    ▼
Frontend: POST /api/tickers/download { tickers: ["AAPL", "NVDA"] }
    |
    ▼
Backend: For each ticker, create per-ticker ZIP
    └── Package all into tickers-bundle.zip
    |
    ▼
...
| ------------------------- | ------------------------------------------ | ----------------------------- |
| Ticker has no data        | Return empty ZIP with empty summary.csv    | Show "No data for {ticker}"   |
...
| Network failure           | N/A (handled by browser)                   | Allow retry                   |
...
- **Ticker sanitization:** Use existing `safe_ticker_component()` to prevent path traversal.
