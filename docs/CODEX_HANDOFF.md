...
Latest phase: First-run preflight command
...
- Branch review confirms `india-market-agents` is clean and 19 commits ahead of `upstream/main` after the first-run preflight command commit.
...
Run `indiamarketagents first-run-check --ticker RELIANCE.NS --date 2026-06-05 --provider <provider>` after configuring an LLM/API key, then continue with the real first-company analysis run in `docs/FIRST_RUN_CHECKLIST.md`. Keep code changes out of scope unless CI or reviewer feedback identifies a specific issue.
