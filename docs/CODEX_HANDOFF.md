...
- Branch review confirms `india-market-agents` is clean and 45 commits ahead of `upstream/main` after the first-run provider-readiness update.
...
- `indiamarketagents first-run-check` now reports a single `Provider readiness` failure when no provider path is ready, instead of also surfacing the configured default provider's missing key.
...
36. First-run provider readiness check:
   - Updated `first-run-check` so the default no-provider-ready path includes a single `Provider readiness` failure with the same provider setup next step as `provider-status`.
...
- `git rev-list --count upstream/main..HEAD`: 45 after the first-run provider-readiness update.
