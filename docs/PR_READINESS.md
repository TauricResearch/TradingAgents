...
Base: `upstream/main`
Branch state: 28 commits ahead of `upstream/main` after the usage playbook command alignment commit.
PR status: open draft PR #1002; GitHub currently reports no status checks in `statusCheckRollup`.
PR body: updated from this file after the usage playbook command alignment commit.
...
- `git status --branch --short`: clean, `india-market-agents...origin/india-market-agents` after the usage playbook command alignment commit.
...
- PR body was updated from this file after the usage playbook command alignment commit.
