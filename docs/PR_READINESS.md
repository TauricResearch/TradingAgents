...
Base: `upstream/main`
Branch state: 29 commits ahead of `upstream/main` after the macOS metadata ignore commit.
PR status: open draft PR #1002; GitHub currently reports no status checks in `statusCheckRollup`.
PR body: updated from this file after the macOS metadata ignore commit.
...
- `git status --branch --short`: clean, `india-market-agents...origin/india-market-agents` after the macOS metadata ignore commit.
...
- PR body was updated from this file after the macOS metadata ignore commit.
