# Cohort Rosters & Coverage

Seed candidate lists — research agents may add well-sourced candidates and must drop any that fail verification (the honest final N is whatever survives QA). Dedup rule: a trader appearing in two cohorts is profiled once, in the cohort marked below; the profile's `style.secondary` captures the other identity.

Status legend: `pending` → `researched` (batch merged) → `qa_ok`.

## C1 — Market Wizards classics (target 30, tier B expected)
