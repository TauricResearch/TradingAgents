...
**Objective**: Ensure the <PROJECT> dashboard server follows a consistent
...
<PROJECT> runs one background service: the dashboard server. It is managed
by `scripts/<SERVICE>.ts` using PID files and log capture.

**Key Principles**:
1.  **PID File Tracking**: Exact process identity. No `ps | grep` fragility.
2.  **Log Capture**: Server stdout/stderr written to `~/.<PACKAGE>/server.log`.
3.  **Kill Port Before Start**: Whatever is on port <PORT> dies. No questions.
...
|---------|------|---------|-------------|
| Dashboard Server | 3000 | `<SERVICE>_PORT` | Hono + HTMX web UI |
...
All service commands use `scripts/<SERVICE>.ts`:

| Command | What |
|---------|------|
| `just start` | Start dashboard server (kills port <PORT> first) |
| `just serve` | Alias for `just start` |
| `just stop` | Stop dashboard server |
| `just restart` | Kill port <PORT>, kill PID, start fresh |
...
```
START:
  1. Kill whatever is listening on port <PORT> (lsof | xargs kill -9)
...
- If port <PORT> is occupied, something is stale or wrong.
...
- **Kill port before start.** Whatever is on port <PORT> dies first. No errors.
...
2. Add PID file: `~/.<PACKAGE>/<service>.pid`
3. Add log file: `~/.<PACKAGE>/<service>.log`
