...
|------------|---------------|-------|
| `justfile` lowercase | Tool default; formatter compatibility | <DATE> |
| `.tsx` for JSX files | Biome parser requirement (JSX in `.ts` fails) | <DATE> |
| `bun` over `node` where both work | Project standard; faster; native TS | <DATE> |
| `Bun.spawn` over `execSync` | Streaming stdin support; no shell quoting bugs | <DATE> |
| Unicode literals over `\uXXXX` in JSX | Hono emits raw HTML; JS escapes only work in `{}` | <DATE> |
| No `new Database()` outside `db.ts` | Factory enforces WAL + pragmas consistently | <DATE> |
| Every deliberate folder has a `README.md` | Prevents mystery; draws boundaries between adjacent dirs | <DATE> |
| **Directory hierarchy** | | |
| `<SRC>/` = production TS (strict) | `tsc --noEmit` enforced; tested code only | <DATE> |
| `<SRC-LIB>/` = shared modules | Imported by `<SRC-SERVER>/` and `<SRC-CLI>/` | <DATE> |
| `<SRC-SERVER>/lib/` = server-only | NOT imported outside `<SRC-SERVER>/` | <DATE> |
| PRs ≤ 30 files per concern | Reviewable, bisectable, merges fast; stack branches for dependencies | <DATE> |
| Pre-PR self-review checklist | 3-gate checklist (mechanical, semantic, docs) before opening PR; catches ~1 real bug per 3 bot flags | <DATE> |
| td ↔ current.md sync at startup | `td list` must match `debriefs/plans/current.md`; stale planning docs are process barnacles | <DATE> |
| `scripts/` = support TS (loose) | Tooling, automation, not shipped | <DATE> |
| `scripts/lab/` = experiments | Loosest types; disposable | <DATE> |
| `scripts/lib/` = shared helpers | Reusable across scripts; pass `just check` | <DATE> |

## Barnacle Removal Record

| Date | Barnacle | Where Found | Why Removed | Removed By |
|------|----------|-------------|-------------|------------|
| <DATE> | `Justfile` (capitalized) | `playbooks/just-playbook.md`, `ci-cd-playbook.md` | Fought `just` formatter; created duplicate file on macOS | claude |
| <DATE> | Empty `<SRC>/` directory | Project root | Claimed to be source root but contained nothing; misled every new agent | claude |
