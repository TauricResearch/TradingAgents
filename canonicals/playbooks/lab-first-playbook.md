...
1. **No imports from `server/` or `cli/` or `<PACKAGE>/`.** Lab scripts are
...
The server lifecycle CLI (`scripts/<SERVICE>.ts`) had a working ASCII
...
`scripts/<SERVICE>.ts` but reverted to hand-rolled ANSI boxes. The
...
## War Story 3: Portfolio and IG History (Session <SESSION-ID>)
