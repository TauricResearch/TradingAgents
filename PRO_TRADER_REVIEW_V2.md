...
4. ~~**Every deploy logs everyone out**~~ — **RETRACTED after investigation.** Sessions are signed with a key derived from a deploy-stable secret and carry a 7-day cookie; the morning session demonstrably survived the 17:40 deploy. The three sign-ins were the *review environment's* browser resetting its own cookie jar between sessions — reviewer error, not a product defect.
