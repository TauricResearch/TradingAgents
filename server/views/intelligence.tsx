...
function _bar(vals, total) {
  ...
  for (var i = 0; i < vals.length; i++) {
    ...
    var color = v.assetClass === 'cash' ? '#3b82f6' :
                ...
                v.assetClass === 'etf' ? '#eab308' :
                v.assetClass === 'crypto' ? '#ef4444' : '#71717a';
    legend += '<span style="margin-right:12px"><span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:' + color + ';vertical-align:middle;margin-right:4px"></span>' + v.assetClass + ' ' + w + '% (' + v.value_gbp.toFixed(0) + ')</span>';
  }
  return bars.join('') + '<div style="margin-top:4px;font-size:0.75em;color:var(--text-dim)">' + legend + '</div>';
}
...
function renderIntel(data) {
  ...
  if (pf.cash_negative) {
    html += '<div class="banner" style="margin-bottom:1rem">\u26a0\ufe0f hledger cash is negative \u2014 more sells recorded than buys in journal. Total and % figures may be misleading until hledger cash is corrected.</div>';
  }
...
function renderPlatforms(data) {
  ...
  for (var pi = 0; pi < data.platforms.length; pi++) {
    ...
    html += '<td style="font-family:Datatype,monospace;font-feature-settings:' + q + 'calt' + q + '1,' + q + 'liga' + q + '1">\u00a3' + _fmt(p.total_value_gbp) + '</td>';
    html += '<td style="font-family:Datatype,monospace;font-feature-settings:' + q + 'calt' + q + '1,' + q + 'liga' + q + '1">' + _fmt(p.weight_pct) + '%</td>';
    html += '<td style="font-family:Datatype,monospace;font-feature-settings:' + q + 'calt' + q + '1,' + q + 'liga' + q + '1">\u00a3' + _fmt(cashValue) + ' <span class="muted">(' + _fmt(p.cash_pct) + '%)</span></td>';
    html += '<td>';
