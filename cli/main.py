...
def save_report_to_disk(final_state, ticker: str, save_path: Path):
    ...
    def _write(path_str: str, content: str):
        with fsspec.open(path_str, "w", encoding="utf-8") as f:
            f.write(content)
    ...
    if final_state.get("market_report"):
        _write(f"{base}/1_analysts/market.md", final_state["market_report"])
    ...
    if final_state.get("fundamentals_report"):
        _write(f"{base}/1_analysts/fundamentals.md", final_state["fundamentals_report"])
    ...
    if final_state.get("investment_debate_state"):
        ...
        if debate.get("bull_history"):
            _write(f"{base}/2_research/bull.md", debate["bull_history"])
            research_parts.append(("Bull Researcher", debate["bull_history"]))
        if debate.get("bear_history"):
            _write(f"{base}/2_research/bear.md", debate["bear_history"])
            research_parts.append(("Bear Researcher", debate["bear_history"]))
        if debate.get("judge_decision"):
            _write(f"{base}/2_research/manager.md", debate["judge_decision"])
    ...
    if final_state.get("trader_investment_plan"):
        _write(f"{base}/3_trading/trader.md", final_state["trader_investment_plan"])
    ...
    if final_state.get("risk_debate_state"):
        ...
        if risk.get("aggressive_history"):
            _write(f"{base}/4_risk/aggressive.md", risk["aggressive_history"])
            risk_parts.append(("Aggressive Analyst", risk["aggressive_history"]))
        if risk.get("conservative_history"):
            _write(f"{base}/4_risk/conservative.md", risk["conservative_history"])
            risk_parts.append(("Conservative Analyst", risk["conservative_history"]))
        if risk.get("neutral_history"):
            _write(f"{base}/4_risk/neutral.md", risk["neutral_history"])
        ...
        if risk.get("judge_decision"):
            _write(f"{base}/5_portfolio/decision.md", risk["judge_decision"])
            sections.append(f"## V. Portfolio Manager Decision\n\n### Portfolio Manager\n{risk['judge_decision']}")

    # Write consolidated report
    header = f"# Trading Analysis Report: {ticker}\n\nGenerated: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    complete_path = f"{base}/complete_report.md"
    _write(complete_path, header + "\n\n".join(sections))
