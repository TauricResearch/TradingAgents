...
def get_user_selections():
    ...
    console.print(
        create_question_box(
            "Step 1: Stock Market", "Select which stock market to analyze", "Auto-detect"
        )
    )
    selected_market = select_market()

    # Step 2: Ticker symbol
    ticker_hint = "VCB, FPT, VNM" if selected_market == "VN" else "SPY"
    console.print(
        create_question_box(
            "Step 2: Ticker Symbol", "Enter the ticker symbol to analyze", ticker_hint
        )
    )
...
def run_analysis():
    ...
    config["market"] = selections.get("market", "auto")
