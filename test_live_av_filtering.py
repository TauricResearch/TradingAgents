"""
Live integration test with real AlphaVantage API data.
Measures actual token reduction on real scanner context.
"""
import os
...
def build_scanner_context_with_real_news(ticker: str) -> str:
    """Build scanner context with real AV news data."""
    
    # Fetch real news from AlphaVantage
    end_date = datetime.now()
    start_date = end_date - timedelta(days=7)
    
    print(f"Fetching real news for {ticker} from AlphaVantage...")
...
"""
    ...
    # Add realistic earnings data (50+ companies)
    companies = [
...
"""
    ...
    # Add real AV news if available
    if articles:
        context += f"\n## IX. REAL ALPHAVANTAGE NEWS (LAST 7 DAYS)\n\n"
        context += f"Total articles fetched: {len(articles)}\n\n"
        
        for i, article in enumerate(articles[:15], 1):  # Show top 15
            title = article.get('title', 'N/A')
            source = article.get('source', 'N/A')
            time_published = article.get('time_published', 'N/A')
            summary = article.get('summary', '')
            
            # Get relevance score for this ticker
            relevance = 0.0
            ...
            context += f"### Article {i}: {title}\n"
            context += f"- Source: {source}\n"
            context += f"- Published: {time_published}\n"
            context += f"- Relevance to {ticker}: {relevance:.2f}\n"
...
def main():
    ...
    print("=" * 80)
    print(f"LIVE INTEGRATION TEST: Real AlphaVantage News for {ticker}")
    print(f"API Key: {api_key[:8]}..." if len(api_key) > 8 else "API Key: SET")
    print("=" * 80)
    
    # Build scanner context with real AV news
    print("\n1. Building scanner context with real AlphaVantage news...")
    original_context = build_scanner_context_with_real_news(ticker)
    
    original_size = len(original_context)
    original_tokens = original_size // 4  # Rough token estimate (4 chars/token)
    
    print(f"\n2. Original context with real AV news:")
    ...
    # Cost savings (Qwen: $0.15 per 1M input tokens)
    ...
    # At scale
    analyses_per_day = 100  # Example: 100 ticker analyses per day
    annual_analyses = analyses_per_day * 365
    annual_savings = cost_saved * annual_analyses
    
    print(f"\n7. ANNUAL COST SAVINGS (at {analyses_per_day} analyses/day):")
    print(f"   - Annual savings: ${annual_savings:.2f}")
    print(f"   - Monthly savings: ${annual_savings/12:.2f}")
    
    # Show sample of filtered output
    print(f"\n8. Filtered context preview (first 1500 chars):")
    print("-" * 80)
    print(filtered_context[:1500])
    print("...")
    print("-" * 80)
    ...
    checks = {
        ...
        "Real news included": "ALPHAVANTAGE NEWS" in original_context,
    }
    ...
    print("\n" + "=" * 80)
    print("TEST COMPLETE - Used real AlphaVantage API data")
    print("=" * 80)
    
    # Save outputs for inspection
    with open("scanner_context_original.txt", "w") as f:
        f.write(original_context)
    with open("scanner_context_filtered.txt", "w") as f:
        f.write(filtered_context)
    
    print("\n✓ Saved outputs to:")
    print("  - scanner_context_original.txt")
    print("  - scanner_context_filtered.txt")
